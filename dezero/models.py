...
class RNNseq2seq(Model):
    def __init__(self, vocab_size, wordvec_size, hidden_size):
        ...
        self.encoder = L.RNNEncoder(V, D, H)
        self.decoder = L.RNNDecoder(V, D, H)
    ...
    def generate(self, src, start_id, sample_size=3):
        h = self.encoder(src)
        sampled = self.decoder.generate(h, start_id, sample_size)
        return sampled

    def reset_state(self):
        self.encoder.time_lstm.lstm.reset_state()
        self.decoder.time_lstm.lstm.reset_state()

class RNNPeekyseq2seq(Model):
    def __init__(self, vocab_size, wordvec_size, hidden_size):
        super().__init__()
        V, D, H = vocab_size, wordvec_size, hidden_size
        self.encoder = L.RNNEncoder(V, D, H)
        self.decoder = L.RNNPeekyDecoder(V, D, H)
